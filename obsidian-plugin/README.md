...
`besttodo_changes.json`, written next to the sync file (no separate setting —
see step 3 below). The view updates immediately (optimistic) and shows a
"syncing…" chip on that task until the phone app has actually picked the
change up — which only happens the next time it's **resumed** (opened or
brought to the foreground), the same way the phone → Obsidian direction only
happens when the app is backgrounded. The app then applies the op, truncates
the journal, and re-syncs, which clears the chip on the next file-change
refresh.
...
2. If it says restricted mode is on, click **Turn on community plugins**.
   (That warning is about third-party code in general; this plugin only
   reads `besttodo_tasks.json` and writes to `besttodo_changes.json` — both
   inside your vault — and never makes any network request.)
...
| A ticked checkbox shows "syncing…" and won't clear | Expected until the phone app resumes — open BestToDo (or bring it to the foreground) once to let it pick up `besttodo_changes.json`. |
