 ...
 * status text to show at this size. Its background still turns red when a
 * meal checkpoint has passed without an entry, matching the full widget.
 */
class FoodDiaryButtonWidgetProvider : HomeWidgetProvider() {
    ...
    override fun onUpdate(
        ...
        val checkpointHours = intArrayOf(8, 13, 20)
        val missed = checkpointHours.indices.any { index ->
            now.get(Calendar.HOUR_OF_DAY) >= checkpointHours[index] &&
                !(dataIsToday && widgetData.getBoolean("food_has_$index", false))
        }
        ...
        appWidgetIds.forEach { widgetId ->
            ...
            views.setInt(
                ...
                "setBackgroundColor",
                if (missed) 0xFFB71C1C.toInt() else 0xFF000000.toInt()
            )
