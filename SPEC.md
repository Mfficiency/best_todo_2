...
### 4.5 Streak (the flames, 0.1.115; three challenges 0.1.157; configurable
goals 0.1.229)
...
goals** since 0.1.229 (`StreakGoal` in `lib/models/streak_goal.dart`, persisted in
...
`recordUncompletion(when, {kind})` (generalized in 0.1.229 — used to be `complete`-only)
...
`_recordStreakPlanning`/`_recordStreakDayCleared` hooks) were retired in 0.1.229 —
...
**Display layer (0.1.229):** `streakFlameInfo(kind)` in
...
**Goal picker (0.1.229):** `StreakGoalDialog` (`lib/ui/streak_goal_dialog.dart`, own
...
icon grows 22→30 px with progress and is outlined + grey while the streak is 0. Tapping
opens `StreakPage` on the kind currently shown. **The cycle is disabled under the test
bindings** (a repeating timer means `pumpAndSettle` never settles, and it also keeps
screenshot runs deterministic) — the check is `WidgetsBinding.instance.runtimeType`
containing "Test"; `StreakFlameButton.debugForceCycle` re-enables it for the test that
covers the cycling.
...
app-wide meaning (0.1.229), so there is nothing generic to backfill for them — they start
...
already met — an unconfigured `create`/`plan` slot is excluded from that check (0.1.229;
