...
| `test/update/` | `flutter test test/update` | In-app updates: `UpdateService` version parsing/comparison and the `github_releases/` folder listing (newest + rollback), the About page's check → download → install flow and its "Go back to ..." button, the background `AutoUpdateChecker` poll (dedup + dismissal) and its "New version available" Yes/No + download-progress dialogs (`auto_update_checker_test.dart`, `auto_update_dialog_test.dart`), `tool/publish_apk.dart` release naming, `tool/stage_local_release.dart` keep-the-newest-two staging, `tool/append_build_time.dart` build-time/duration CHANGELOG notes and `build_history.json` recording (`append_build_time_tool_test.dart`) |
