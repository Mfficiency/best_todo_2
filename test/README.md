...
| `test/update/` | `flutter test test/update` | In-app updates: `UpdateService` version parsing/comparison, the `github_releases/` folder listing (newest + rollback), and the background download (Android's `DownloadManager`, faked through `downloadChannelOverride`: request shape, wire-status mapping, polling to a terminal status, pending-download bookkeeping for a resume after restart), the About page's check → background download → install flow and its "Go back to ..." button, the background `AutoUpdateChecker` poll (dedup + dismissal) and its "New version available" Yes/No dialog + `downloadUpdateInBackground`'s no-APK failure snackbar (`auto_update_checker_test.dart`, `auto_update_dialog_test.dart`), `tool/publish_apk.dart` release naming, `tool/stage_local_release.dart` keep-the-newest-two staging + its `--version` override for a Best Music build (`stage_local_release_tool_test.dart`), `tool/append_build_time.dart` build-time/duration CHANGELOG notes and `build_history.json` recording, including its `--app music` mode writing CHANGELOG_MUSIC.md/reading MUSIC_VERSION instead (`append_build_time_tool_test.dart`), and `tool/bump_version.dart` bumping pubspec.yaml + CHANGELOG.md (or, with `--music`, MUSIC_VERSION + CHANGELOG_MUSIC.md) while preserving CHANGELOG_MUSIC.md's preamble paragraph (`bump_version_tool_test.dart`) — Best Music versions and changelogs independently of BestToDo since the split (CLAUDE.md/SPEC.md §10.6f) |
...
| `test/music/` | `flutter test test/music` | Music Player: `Track`/`MusicPlaylist` model JSON round-trips, `MusicLibraryService` (recursive folder scan honoring `Config.musicExcludedSubfolders`, best-effort mp3 ID3 tag reads falling back to the filename, `listSubfolders`, cache persistence), `MusicPlaylistService` (Favorites/"Don't really like" system playlists, custom playlist create/delete, the Efraimidis–Spirakis weighted shuffle favoring favorites and burying disliked tracks), `M3uPlaylistService` (M3U/M3U8 parsing and exact-path/basename track matching for a Samsung-Music-style import), `SubsonicClient` (auth token/salt, stream URL shape, fails closed when unconfigured), `MusicPlayerPage`'s folder-not-chosen prompt vs. Library/Playlists tabs, `MusicAudioHandler.reorderQueue`/`toggleShuffle` (queue drag-reorder, shuffle-tail-and-restore, `music_audio_handler_test.dart`), `NowPlayingPage`'s shuffle/Queue app-bar buttons + navigation to the empty `QueuePage` (`now_playing_page_test.dart`), `Track`'s genre/year/dateAdded/playCount round-trip + `copyWith` (`track_model_test.dart`), `MusicLibraryService`'s dateAdded/playCount preservation across a rescan and `incrementPlayCount` (`music_library_service_test.dart`), `decodeMp3Tags`'s never-throws fallback (`music_metadata_extractor_test.dart`), `PlaylistRuleSet`/`RuleCondition` matching (AND/OR/NOT semantics) and JSON round-trips (`playlist_rule_test.dart`), `MusicPlaylistService.resolvedTracks`/`smartPlaylists`/`createRulePlaylist`/`updateRulePlaylist` (`music_playlist_service_test.dart`), `RulePlaylistEditorPage`'s create/validate/edit flows (`rule_playlist_editor_page_test.dart`), hand-built playlist create/add-to/remove flows in `music_player_page_test.dart`'s "hand-built playlists" group, `MusicLibraryService.rescan`'s `onTrackScanned` progress callback and `updateTrackMetadata` (sets fields + `metadataEdited`, survives a later rescan, `music_library_service_test.dart`), `MusicMetadataScanPage`'s live scan progress/genre-year status/rescan-again flow (`music_metadata_scan_page_test.dart`), `TrackMetadataPage`'s view/edit/validate flow (including its Tags field) + Now Playing's disabled-until-playing info button (`track_metadata_page_test.dart`, `now_playing_page_test.dart`), `MusicMetadataCsv.encode`/`decode` (quoting, header-name column lookup, tolerant of reordered/missing columns and no trailing newline, the `tags` column's `; `-joined multi-value cell) + `MusicLibraryService.applyMetadataRows` (blank-cell-preserves-existing semantics, unmatched-id skip, persistence, `music_metadata_csv_test.dart`), `splitArtistCredit` grouping a "feat."/"ft."/"featuring" credit under its main artist (`artist_utils_test.dart`) and the Artists tab using it to merge those credits with a "feat. ..." trailing label, the Tags tab grouping tracks by their (possibly multiple) user-assigned tags with an "Untagged" bucket (`music_player_page_test.dart`), and Best Music's Wishlist tool (`music_wishlist_page_test.dart`): wish-flagged-only filtering, each row's checkbox-plus-title tile (toggling it marks done/persists without opening the editor), opening an item's description/priority/tags editor, add/edit/delete round-tripping through `StorageService` in the same `Task`/`tasks.json` shape BestToDo's own Wishlist uses, (`Config.isBestMusic`) never importing BestToDo's own Todo.md backlog even on a fresh install, and its "does not sync with BestToDo" group proving it never reads or writes `SharedWishlistStore`'s shared external-storage file |
...
| `test/tools/` | `flutter test test/tools` | Auxiliary tools: export/import + analytics CSVs, usage-data service, startup-times page, countdown timer model + milestone notifications (model & dialog), timer date picker (`lib/utils/date_time_format.dart`), chronize page, CI test report model/parser + Test Results page, wishlist Todo.md import migration (including `Config.isBestMusic` skipping it entirely), stable backlog uids + shipped-wish auto-completion (`wishlist_autocomplete_test.dart`), wishlist page (filtered view, priority/delete swipes), the Wishlist view's Filtering rules tag restriction (`wishlist_filter_rules_test.dart`), BestToDo's Wishlist "Connect" banner pushing wish items out to the shared external-storage file `SharedWishlistStore` reads/writes (`wishlist_page_test.dart`; Best Music's own Wishlist no longer touches this file at all — see `test/music/music_wishlist_page_test.dart`'s "does not sync with BestToDo" group), the Waiting for Approval view's Filtering rules tag restriction on top of the pending gate (`waiting_approval_filter_rules_test.dart`), changelog parser + update heatmap (toggle, day selection) + `ChangelogPage`'s app-awareness (`assetPath`/`showStoryPoster`, so Best Music's drawer entry renders CHANGELOG_MUSIC.md without BestToDo's story poster), Productivity Stats item-activity heatmap colour scale (`stats_activity_heatmap_test.dart`), failure-dot acknowledgement (`test_report_service_test.dart`), Weekly Hours Planner (`WeeklyHoursPlan` model + Friday flexitime carryover, `WeeklyHoursService` persistence, the vertical week grid's drag-to-resize block handles + reset button + Google Calendar overlay, `weekly_hours_planner_test.dart`) and its Google Calendar .ics import/RRULE expansion (`google_calendar_service_test.dart`), and Fitness Activity's manual weight/personal-bests tracking — `WeightEntry`/`PersonalBest` json round-trip + `HealthTrackingService` persistence (`health_tracking_service_test.dart`) and the add/edit/delete UI on the Fitness Activity page (`health_tracking_page_test.dart`) |
...
- `lib/ui/home_page.dart`, `lib/ui/task_tile.dart`, `lib/ui/settings_page.dart`,
  ...
  + **music** for either file's Music Player tool/section wiring)
- `lib/models/track.dart`, `lib/models/music_playlist.dart`, `lib/models/playlist_rule.dart`,
  `lib/services/music_library_service.dart`, `lib/services/music_playlist_service.dart`,
  `lib/services/music_metadata_extractor.dart`, `lib/services/music_metadata_csv.dart`,
  `lib/services/music_audio_handler.dart`,
  `lib/services/music_player_service.dart`,
  `lib/services/music_widget_service.dart`, `lib/services/m3u_playlist_service.dart`,
  `lib/services/subsonic_client.dart`, `lib/utils/artist_utils.dart`,
  `lib/ui/music_player_page.dart`,
  `lib/ui/rule_playlist_editor_page.dart`, `lib/ui/music_metadata_scan_page.dart`,
  `lib/ui/track_metadata_page.dart`, `lib/ui/now_playing_page.dart`,
  `lib/ui/queue_page.dart`, `tool/scan_music_metadata.dart` → core + **music** (+ **home**, whose
...
- `lib/utils/wish_priority.dart` (the wishlist priority labels/rank/setter
  shared by `wishlist_page.dart` and `music_wishlist_page.dart`) → core +
  **tools** + **music**
- `lib/ui/music_wishlist_page.dart` (Best Music's Wishlist tool, drawer entry
  in `music_player_page.dart`) → core + **music**
- `lib/services/shared_wishlist_store.dart`, `lib/ui/wishlist_sync_banner.dart`
  (cross-app wishlist sync between BestToDo's and Best Music's Wishlist
  pages, `Config.wishlistSyncBannerDismissed`) → core + **tools** + **music**
