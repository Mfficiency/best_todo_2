...
| `test/sms/` | `flutter test test/sms` | SMS daily report: `SmsRecipient`/`SmsReportConfig` JSON round-trip, per-recipient enable flag + `activeRecipients` filtering, `SmsMessageTemplate` round-trip, named-template resolution + per-recipient send-time fallback (`SmsReportConfig.templateBodyFor`/`.timeFor`), the per-time-slot alarm set derived from a config (`SmsReportScheduler.activeSlotMinutes`), Settings recipient rows (pause switch, edit keeps the flag, custom send time), named-template add/remove |
